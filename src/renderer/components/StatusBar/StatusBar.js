...
const socket = io('http://${process.env.CEA_URL}');
    ...
    const getJobOutput = async () => {
      try {
        const resp = await axios.get(
          `http://${process.env.CEA_URL}/server/streams/read/${job.id}`,
          null,
          { responseType: 'text' }
        );
...
const cancelCeaJob = (job) => {
  axios.post(`http://${process.env.CEA_URL}/server/jobs/cancel/${job.id}`);
};
